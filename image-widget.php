<?php
/*
Plugin Name: Image in the Widget
Plugin URI: http://wordpress.org/extend/plugins/image-in-the-widget/
Description: A simple widget that uses the native WordPress media manager to add images to widget of your site.
Author: Sapian Technologies
Version: 2.0.1
Author URI: http://www.sapiantech.com
*/
...
function sap_load_image_widget() {
	register_widget('sap_Image_Widget');
}
add_action('widgets_init', 'sap_load_image_widget');

/**
 * sap_Image_Widget class
 **/
class sap_Image_Widget extends WP_Widget {

	const VERSION = '4.1';

	const CUSTOM_IMAGE_SIZE_SLUG = 'sap_image_widget_custom';

	/**
	 * sap Image In The Widget constructor
	 *
	 * @author Modern sap, Inc.
	 */
	function sap_Image_Widget() {
		...
		$this->WP_Widget('widget_sp_image', __('Image In The Widget', 'image_widget'), $widget_ops, $control_ops);
	...
	function admin_setup() {
		wp_enqueue_media();
		wp_enqueue_script( 'sap-image-widget', plugins_url('resources/js/image-widget.js', __FILE__), array( 'jquery', 'media-upload', 'media-views' ) );

		wp_localize_script( 'sap-image-widget', 'sapImageWidget', array(
	 ...
	 * @author Modern sap, Inc. (Matt Wiebe)
	 **/
